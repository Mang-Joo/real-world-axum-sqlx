...
pub async fn user_login(
...
) -> config::Result<UserResponse> {
    let user = find_by_email(&login_request.email.unwrap(), Arc::clone(&app_state))
        .await
        .map_err(|err| anyhow!(err))?;

    if user
        .not_verify_password(login_request.password.unwrap(), &ArgonHash::default())
        .await
    {
    ...
    let token = jwt_encoder.encode_jwt(&user).await?;
...
pub struct LoginRequest {
    #[validate(
        ...
        email(message = "Must be form email.")
    )]
